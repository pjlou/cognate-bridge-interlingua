...
export default function StudyPage() {
  ...
  const bands = useAsync(
    ...
    [bridge, effectiveSkipEnglish, effectiveOnlyEnglish],
  ...
  const due = useAsync(
    ...
    [bridge, band, effectiveSkipEnglish, effectiveOnlyEnglish],
  ...
  const queue = useAsync(
    ...
    [bridge, band, effectiveSkipEnglish, effectiveOnlyEnglish, sessionSize],
  ...
  const grade = useCallback(
    async (nextGrade: VocabularyGrade) => {
      ...
      setSaveError(null);
      try {
        await recordReview(card.id, nextGrade, card.tier === 2 ? 2 : 1);
        cancelBridgeSpeech();
  ...
  const removeFromDeck = useCallback(async () => {
    ...
    setSaveError(null);
    try {
      await setVocabularyRemoved(card.id, card.tier === 2 ? 2 : 1, true);
      cancelBridgeSpeech();
      const kind = card.queue_kind ?? 'new';
